...
const errorReducer = (
  ...
  switch (action.type) {
    case "ADD_ERROR": {
      ...
      const updatedHistory = [...state.errorHistory, newError].slice(
        -state.maxHistorySize,
      );

      const shouldSetGlobal =
        newError.severity === ErrorSeverity.CRITICAL && !state.globalError;
    ...
    case "REMOVE_ERROR": {
      ...
      const updatedErrors = state.errors.filter(
        (error) => error.context.errorId !== errorId,
      );

      const globalError =
        state.globalError?.context.errorId === errorId
          ? null
          : state.globalError;
