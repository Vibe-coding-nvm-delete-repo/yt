...
export const runLayoutWidthTests = () => {
  ...
  layoutWidthTests.forEach((test, index) => {
    ...
    const testPassed =
      test.expectedClass === "max-w-4xl" && test.previousClass === "max-w-7xl";

    if (testPassed) {
      passed++;
      console.log(
        `✓ Test ${index + 1} passed: ${test.component} uses ${test.expectedClass}`,
      );
    } else {
      failed++;
  ...
  console.log("\nExpected behavior:");
  console.log(
    "- All layout containers (header, nav, main, footer) use max-w-4xl (~896px)",
  );
