...
export const usePerformantSettings = (subscribeToKeys?: (keyof AppSettings)[]) => {
  ...
  const memoizedSettings = useMemo(() => {
    ...
    if (previousSettingsRef.current && settingsHashRef.current === currentHash) {
      return previousSettingsRef.current; // Return same object reference
    }
...
export const useSettingKey = <K extends keyof AppSettings>(
  ...
  const updateValue = useCallback((newValue: AppSettings[K]) => {
    performanceStorage.batchUpdate({ [key]: newValue } as Partial<AppSettings>);
  }, [key]);
...
export const useModelSelectionOptimized = () => {
  const { settings } = usePerformantSettings(['selectedModel', 'availableModels']);
...
export const usePinnedModelsOptimized = () => {
  const { settings } = usePerformantSettings(['pinnedModels', 'availableModels']);
...
export const useApiValidationOptimized = () => {
  const { settings } = usePerformantSettings(['isValidApiKey', 'lastApiKeyValidation']);
