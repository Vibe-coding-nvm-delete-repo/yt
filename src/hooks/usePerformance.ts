...
export const useRenderTracker = (
  ...
  const renderInfoRef = useRef<RenderInfo>({
    ...
    componentName,
    rapidRenderCount: 0,
    lastRapidRenderReset: getCurrentTime(),
  });
  ...
  if (enabled) {
    ...
    const timeSinceLastRender = currentTime - info.lastRenderTime;
    ...
    if (currentTime - info.lastRapidRenderReset > rapidRenderWindowMs) {
      info.rapidRenderCount = 0;
      info.lastRapidRenderReset = currentTime;
    }
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      info.renderCount = 0;
      info.rapidRenderCount = 0;
      info.lastRapidRenderReset = getCurrentTime();
  ...
  const getMetrics = useCallback((): PerformanceMetrics | null => {
    ...
    return {
      ...
      averageRenderInterval:
        info.renderCount > 1
          ? (currentTime - info.lastRapidRenderReset) / info.renderCount
          : 0,
      rapidRenders: info.rapidRenderCount,
      mountTime: info.lastRapidRenderReset,
      lifespan: currentTime - info.lastRapidRenderReset,
    };
  }, [enabled, info]);
...
export const useComponentLifecycle = (
  ...
  const mountTimeRef = useRef<number>(getCurrentTime());
  const metricsRef = useRef<PerformanceMetrics | null>(null);

  useEffect(() => {
    if (!enabled) return;

    const mountTime = getCurrentTime();
    mountTimeRef.current = mountTime;

    if (logMount) {
      console.warn(
        `🔧 [Lifecycle] ${componentName} mounted at ${new Date(mountTime).toLocaleTimeString()}`,
      );
    }

    return () => {
      const unmountTime = getCurrentTime();
  ...
  const getLifecycleMetrics = useCallback(() => {
    ...
    return {
      mountTime: mountTimeRef.current,
      currentLifespan: currentTime - mountTimeRef.current,
...
export const useOperationTimer = (
  ...
  const endTimer = useCallback(
    (operationName: string, logResult: boolean = true) => {
      if (!enabled) return 0;
      ...
      if (logResult) {
        ...
        console.warn(
          `${emoji} [Performance] ${operationName}: ${duration.toFixed(2)}ms`,
        );
      }
...
export const usePerformanceMonitor = (
  ...
  const renderTracker = useRenderTracker(componentName, {
    enabled: enabled && trackRenders,
  });
  const lifecycle = useComponentLifecycle(componentName, {
    enabled: enabled && trackLifecycle,
  });
  const memory = useMemoryMonitor(componentName, {
