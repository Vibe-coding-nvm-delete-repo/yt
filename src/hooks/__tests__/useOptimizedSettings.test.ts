...
const mockStorage = optimizedSettingsStorage as jest.Mocked<
  typeof optimizedSettingsStorage
>;
...
describe("useOptimizedSettings", () => {
  ...
  it("should subscribe to all settings changes when no keys specified", () => {
    ...
    expect(mockStorage.subscribe).toHaveBeenCalledWith(expect.any(Function), {
      keys: undefined,
      immediate: false,
    });
  });
  ...
  it("should subscribe to specific keys when provided", () => {
    const subscribeKeys: (keyof typeof mockSettings)[] = [
      "openRouterApiKey",
      "selectedModel",
    ];
